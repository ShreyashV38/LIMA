 ...
 */
// gcc -Wall -Wextra -g -I./include src/main.c src/core/*.c src/data_structures/*.c src/ui/*.c src/vfs/*.c -o lima.exe
...
static int cmd_mkdir(const char *name) {
  ...
  if (!ok) {
    fprintf(
        stderr,
        "mkdir: cannot create directory '%s': already exists or invalid name\n",
        name);
    return 1;
  }
  return 0;
...
static int cmd_touch(const char *name) {
  ...
  if (!ok) {
    fprintf(stderr,
            "touch: cannot create file '%s': already exists or invalid name\n",
            name);
    return 1;
  }
  return 0;
...
static int cmd_edit(const char *filepath) {
  ...
  if (file_gb) {
    ...
    if (!vfs_touch(g_vfs, filepath)) {
      fprintf(stderr,
              "edit: cannot open or create '%s'. It might be a directory.\n",
              filepath);
      return 1;
    }
...
static int cmd_paste(void) {
  if (!vfs_clipboard_paste(g_clipboard, g_vfs)) {
    fprintf(
        stderr,
        "paste: clipboard is empty or name collision in current directory\n");
    return 1;
  }
...
static int cmd_load(const char *host_path) {
  ...
  if (!g_session) {
    fprintf(stderr, "load: warning — session could not be recreated; undo/redo "
                    "unavailable\n");
  }
...
static int cmd_help(void) {
  ...
  printf("    paste               Paste from clipboard into current "
         "directory\n\n");
  printf("  History:\n");
...
static int dispatch(char *line) {
  ...
  if (strcmp(cmd, "exit") == 0 || strcmp(cmd, "quit") == 0) {
    ...
    fprintf(stderr,
            "'%s': unknown command.  Type 'help' for a list of commands.\n",
            cmd);
  }
